...
class LiFT:
    def __init__(self, tel,
        ...
        if self.basis.shape[0] != self.tel.resolution and self.basis.shape[1] != self.tel.resolution:
            raise ValueError('The modal basis resolution (%i,%i) does not match the telescope resolution %i',
                             self.basis.shape[0], self.basis.shape[1], self.tel.resolution)
        ...
        if ang_pixel_arcsec is not None:
            ...
            self.zeroPaddingFactor = (
                self.tel.src.wavelength / self.tel.D) * (1/(self.ang_pixel_rad))
            print('Using user-input PSF sampling:%.1f Pixels/FWHM ' %
                  self.zeroPaddingFactor)

        else:
            self.zeroPaddingFactor = self.det.psf_sampling
            self.ang_pixel_rad = (
                self.tel.src.wavelength / self.tel.D) / self.zeroPaddingFactor
            self.ang_pixel_arcsec = self.ang_pixel_rad * ((180/np.pi)*3600)
            print('Using detector PSF sampling:%.1f Pixels/FWHM ' %
                  self.zeroPaddingFactor)
    ...
    def print_modes(self, A_vec):
        ...
        for i in range(A_vec.shape[0]):
            val = A_vec[i]
            if val is not None and not xp.isnan(val):
                val = xp.round(val, 4)
            print('Mode #', i, val)
    ...
    def generateLIFTinteractionMatrices(self, coefs, modes_ids, flux_norm=1.0):
        ...
        if not self.numerical:
            ...
            for i in modes_ids:
                ...
                derivative = 2 * \
                    set_binning((xp.real(1j * buf * Pd)), aux_oversampling) * k
            ...
            for i in modes_ids:
                self.tel.OPD = (np.squeeze(
                    self.basis[:, :, i]) * delta) + initial_OPD
                ...
                self.tel.OPD = - \
                    (np.squeeze(self.basis[:, :, i]) * delta) + initial_OPD
        ...
        # sum all spectral interaction matricies
        return xp.dstack(H).sum(axis=2)
    ...
    def Reconstruct(self, PSF_inp, R_n, mode_ids, A_0=None, verbous=False, optimize_norm='sum', check_convergence=True, numerical=None):
        ...
        if self.gpu:
            xp = cp
            def convert(x): return cp.asnumpy(x)
        else:
            xp = np
        ...
        def normalize_PSF(PSF_in):
            if optimize_norm is not None and optimize_norm is not False:
                if optimize_norm == 'max':
                    return (PSF_in / PSF_in.max(), PSF_in.max())
                if optimize_norm == 'sum':
                    return (PSF_in / PSF_in.sum(), PSF_in.sum())
            else:
                return (PSF_in, 1.0)

        # initial PSF assumtion, normalized to 1.0
        PSF_0, flux_cap = normalize_PSF(PSF_from_coefs(A_est))
        ...
        def criterion(i): return xp.abs(C[i] - C[i - 1]) / C[i]
        def coefs_norm(v): return xp.linalg.norm(v[modes], ord=2)

        def inverse_Rn(Rn):
            return 1. / xp.clip(Rn.flatten(), a_min=1e-6, a_max=Rn.max())

        if R_n is not None:
            # basically if it's 'model' or 'iterative':
            if isinstance(R_n, str):
                inv_R_n = inverse_Rn(PSF_0 * flux_scale +
                                     self.det.readoutNoise ** 2)
            else:
                inv_R_n = inverse_Rn(xp.array(R_n))
        else:
            inv_R_n = inverse_Rn(xp.ones_like(
                PSF) * self.det.readoutNoise ** 2)
        ...
        for i in range(self.iterations):
            ...
            H = self.generateLIFTinteractionMatrices(
                A_est, modes, flux_scale / flux_cap)
