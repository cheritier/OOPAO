...
class Detector:
    def __init__(self,nRes:int = None,readoutNoise:float=0,photonNoise:bool=False,backgroundNoise:bool=False,backgroundNoiseMap:float = None,QE:float=1,psf_sampling:float = 2):
        '''
    ...
    def integrate(self,frame):
        ...
        if self.readoutNoise!=0:
            self.frame += np.int64(np.round(self.random_state_readout_noise.randn(self.frame.shape[0],self.frame.shape[1])*self.readoutNoise))                
