...
#    
    ...
    gaussian_m4_commands = np.zeros([5352,5352])
    for i in range(6):
        print('fitting petal'+str(i)+'/6..')
        petal_m4 = dm.modes[:,i*892:(i+1)*892]
        projector = np.linalg.pinv(petal_m4)        
        gaussian_m4_commands[i*892:(i+1)*892,i*892:(i+1)*892] = projector @ dm_gaussian.modes[:,i*892:(i+1)*892]
