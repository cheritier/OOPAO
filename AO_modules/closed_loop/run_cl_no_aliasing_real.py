# -*- coding: utf-8 -*-
"""
Created on Mon Nov 16 15:33:03 2020
...
def run_cl_real_no_aliasing(param,obj):
...
#    case with no mis-registration
    ...
    if obj.display:
        ...
        psf_ho=ax5.imshow(np.log(np.abs(obj.tel.PSF)))
        plt.colorbar(psf_ho)
        plt.title('WFS frame')
        
        ax6 = plt.subplot(2,3,6)
        wfs_cl_aliasing=ax6.imshow(obj.wfs.pyramidSignal_2D)
        plt.colorbar(wfs_cl_aliasing)
        plt.title('Aliasing Reconstructed by WFS [rad]')
...
#%%   
    ...
    for i_loop in range(nLoop):
        ...
        fitting_OPD,corr_OPD,c   = getFittingError(obj.tel.OPD,obj.projector,obj.basis, display = False)
 
        obj.tel.OPD = fitting_OPD
        
        obj.tel*obj.wfs
        
        aliasing_WFS    = obj.wfs.pyramidSignal
        phase_aliasing  = 2*obj.tel.pupil*np.reshape(np.matmul(dm_cl.modes,np.matmul(reconstructor,aliasing_WFS)),[obj.tel.resolution,obj.tel.resolution])*2*np.pi/obj.ngs.wavelength
        
        # Remove  fitting error 
        obj.tel.OPD = atm.OPD
        ...
        residualPhase = np.reshape(obj.tel.OPD,obj.tel.resolution**2)
        try:
            modal_coefficients_turb[i_loop,:] = obj.projector @ residualPhase
        except:
            if i_loop == 0:
        ...
        residualPhase = np.reshape(obj.tel.OPD,obj.tel.resolution**2)
        try:
            modal_coefficients_res[i_loop,:] = obj.projector @ residualPhase
        except:
            if i_loop==0:
                print('Error - no projector for the modal basis..')
        ...
        if obj.display==True:
            ...
            # turbulence phase
            tmp = obj.tel.PSF_trunc/obj.tel.PSF_trunc.max()
            psf_ho.set_data(np.log(np.abs(tmp)))
            psf_ho.set_clim(vmin=-5,vmax=0) 
            
            wfs_cl_aliasing.set_data(phase_aliasing)
            wfs_cl_aliasing.set_clim(vmin=phase_aliasing.min(),vmax=phase_aliasing.max())
