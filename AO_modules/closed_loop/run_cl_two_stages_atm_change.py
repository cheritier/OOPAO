...
def run_cl_two_stages_atm_change(param,obj,start_change,n_transition,func_change_wind_speed = None ,func_change_wind_direction = None ,func_change_r0 = None,speed_factor = 2,filename_phase_screen = None, extra_name = '', destination_folder = None, get_le_psf = False):
    
    ref_wind_direction  = np.asarray(obj.atm_fast.windDirection)
    ref_wind_speed      = np.asarray(obj.atm_fast.windSpeed)
    ref_r0              = np.copy(obj.atm_fast.r0)    
    ref_seeing              = np.copy(obj.atm_fast.seeingArcsec)    
    end_change = start_change + n_transition
    nLoop = param['nLoop']
...
#%%   
    ...
    for i_loop in range(nLoop):
        if i_loop>start_change and count_change< n_transition:
                # update wind direction
                if func_change_wind_direction is not None:
                    obj.atm_fast.windDirection  = list(ref_wind_direction+func_change_wind_direction[:,count_change])
                # update wind speed                    
                if func_change_wind_speed is not None:                    
                    obj.atm_fast.windSpeed      = list(ref_wind_speed+func_change_wind_speed[:,count_change])
                # update r0
                if func_change_r0 is not None:
                    obj.atm_fast.r0             = ref_r0+func_change_r0[count_change]

                count_change+=1
                obj.atm_fast.print_atm()
