...
#                
        ...
        if obj.printPetals:
            if obj.dm.isM4:
                ...
                open_loop_modal_command = np.matmul(calib_cl.M,wfsSignal)
                open_loop_modal_command -= np.mean(open_loop_modal_command)
                open_loop_LO_filtered   = M2C_cl[:,2:] @ open_loop_modal_command[2:]
                for i_petal in range(6):                   
                    petal               = obj.tel.getPetalOPD(i_petal)*1e9             
                    petal_nm            = np.round(np.mean(petal[np.where(obj.tel.petalMask == 1)]),5)
                    mean_command_value  = np.mean(open_loop_LO_filtered[i_petal*892:(i_petal*892+811)])

                    print('Petal '+str(i_petal+1) +': ' + str(petal_nm))
                    print('mean command '+str(i_petal+1) +': ' + str(mean_command_value))
