...
tel = Telescope(resolution           = 16*n_subaperture,                          # resolution of the telescope in [pix]
                diameter             = 8,                                        # diameter in [m]        
                samplingTime         = 1/500,                                   # Sampling time in [s] of the AO loop
                centralObstruction   = 0.,                                      # Central obstruction in [%] of a diameter
                fov                  = 80/206256,                                # field of view in rad   
...
# create the Lase Guide Star object

n = 20
Na_profile = np.zeros([2,n])
Na_profile[0,:] = np.linspace(90000,100000,n)
Na_profile[1,:] = 1/n
   

ngs = Source(optBand                        ='Na',           # Optical band (see photometry.py)
             magnitude                      = 10,            # magnitude
             laser_coordinates              = [0,0],        # coordinates of the laser launch telescope in [m]
             Na_profile                     = Na_profile,   # Na profile ( altitude and )
             FWHM_spot_up                   = 0.05,         # FWHM of the spots in arcsec
             coordinates                    = [0,0])        # coordinates of the star [zenith in arcsec, azimuth angle in deg]



# create the Scientific Target object
src = Source(optBand   = 'Na',           # Optical band (see photometry.py)
             magnitude = 4)             # Source Magnitude
...
atm = Atmosphere(telescope     = tel,                               # Telescope                              
                 ...
                 windSpeed     = [5   ], # Wind Speed in [m]
                 windDirection = [0   ], # Wind Direction in [degrees]
                 altitude      = [0 ]) # Altitude Layers in [m]# initialize atmosphere with current Telescope
...
dm = DeformableMirror(telescope  = tel,                        # Telescope
                    nSubap       = 10,              # number of subaperture of the system considered (by default the DM has n_subaperture + 1 actuators to be in a Fried Geometry)
...
wfs = ShackHartmann(nSubap              = n_subaperture,        # number of subaperture
              ...
              padding_extension_factor  = 1)                 # Flag to use a shannon sampling for the shack-hartmann spots



# ngs*tel
# wfs_ngs = ShackHartmann(nSubap          = 1,        # number of subaperture
#               telescope                 = tel,                  # telescope object
#               lightRatio                = 0.5,                  # flux threshold to select valid sub-subaperture
#               binning_factor            = 1,                    # binning factor
#               is_geometric              = True,                # Flag to use a geometric shack-hartmann (direct gradient measurement)
...
# Modal interaction matrix
calib_KL = CalibrationVault(calib_zonal.D@M2C_KL[:,:100])
...
for i in range(nLoop):
    ...
    print('Loop'+str(i)+'/'+str(nLoop)+' Turbulence: '+str(total[i])+' -- Residual:' +str(residual[i])+ '-- SR'+str(SR[i])+ '\n')
